...
class ALU(object):
    ...
    ALU_OPS = {
        OpCode.ADD: lambda x, y: x + y, OpCode.SUB: lambda x, y: x - y,
        OpCode.MUL: lambda x, y: x * y, OpCode.DIV: lambda x, y: x // y,
        #
...
class CPU(MVCListenable):
    ...
    def step(self):
        ...
        instruc_pred = self.condition & instr.cond
        if instruc_pred:
           l_value = instr.reg_src1
           r_value = instr.reg_src2 + instr.offset
           increment = self.pc.get() + 1
           self.pc.put(increment)
           result, condition = self.alu.exec(instr.op, l_value, r_value)

           if instr.op == OpCode.STORE:
               reg_value = self.registers[instr.reg_target].get()
               self.memory.put(result, reg_value)

           elif instr.op == OpCode.LOAD:
               target = self.registers[instr.reg_target]
               memory = self.memory.get(result)
               target.put(memory)
               ...
               self.condition = CondFlag(condition)
